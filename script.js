...
const controlsWrapper = document.querySelector('[data-slide="constrols-wrapper"]');
...
function createSlideClones() {
    ...
    const pnultimateSlide = slideItems[slideItems.length - 2].cloneNode(true)
    pnultimateSlide.classList.add('slide-cloned')
    pnultimateSlide.dataset.index = -2


    slidelist.append(firstSlide)
    slidelist.append(secondSlide)
    slidelist.prepend(lastSlide)
    slidelist.prepend(pnultimateSlide)
...
function onMouseDown(event, index) {
    ...
    state.currentPoint = event.clientX - state.savedPositio
    state.currentSlideIndex = index
...
function setAutoPlay(){
    if (state.autoPlay) { slideInterval = setInterval(function() {
        setVisibleSlide({index: state.currentSlideIndex + 1, animate: true})
    }, state.timeInterval)}
